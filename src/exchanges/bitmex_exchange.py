...
class BitmexExchange(BaseExchange):
    ...
    def get_subscribe_message(self, symbol: str) -> Dict:
        # Symbol is already in BitMEX format from configuration
        message = {
            'op': 'subscribe',
            # Subscribe to both quote (bid/ask) and trade (last price) data
            'args': [f'quote:{symbol}', f'trade:{symbol}']
        }
        return message
    ...
    async def _handle_quote_update(self, message: Dict):
        ...
        for data in data_list:
            # Get symbol - it's already in BitMEX format
            ...
            # Use BitMEX symbol directly - mapping will handle display symbol conversion
            price_data = self.format_price_data(bitmex_symbol, price, bid, ask, timestamp)
    ...
    def normalize_symbol(self, symbol: str) -> str:
        """Normalize symbol format - keep standard format for output."""
        return symbol.upper()